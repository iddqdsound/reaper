...
for SelectedItemIndex = 0,SelectedItemCount-1 do
    ...
    local SourceBaseName = string.gsub(SelectedTakeSourceName,".*[/\\]","") --Removes the path name from the source file name
       reaper.GetSetMediaItemTakeInfo_String(ActiveTakeInItem,"P_NAME",SourceBaseName,true) --renames the active take name by the above variablereaper.GetSetMediaItemTakeInfo_String(ActiveTakeInItem,"P_NAME",SourceBaseName,true) --renames the active take name by the above variable
